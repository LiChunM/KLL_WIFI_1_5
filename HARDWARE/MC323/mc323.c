...
void atk_8266_wifiap_init(void)
{
	...
	delay_ms(1000);
	atk_8266_at_response(1);
	sprintf((char*)p,"AT+CWSAP=\"%s\",\"%s\",11,0",wifiap_ssid,wifiap_password); 
...
void ProcessingTheirReply(u8 *RtuReplyBuf)
{
	u8 res=0;
	u8 *p1;
	if(RtuReplyBuf[0]==0x7E&&RtuReplyBuf[1]==0x7E)
		{
			DataSensoCheck=1;
		}
}
...
resend1:
	...
	res=atk_8266ReviceCheckOK(100);
	if(res==1)
		{
			if(SystemFlow==1)return 2;
			stime++;
			if(stime>=200)return 0;
			else goto resend1;
		}
	SystemFlow=3;
	return 1;
